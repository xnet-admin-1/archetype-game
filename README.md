# Archetype

Collaborative storytelling game for Android where 2-4 players take turns building stories together, powered by AI.

[![Built with Pollinations](https://img.shields.io/badge/Built%20with-Pollinations-8a2be2?style=for-the-badge&logo=data:image/png;base64,&logoColor=white&labelColor=6a0dad)](https://pollinations.ai)

## Features

- **Solo Mode** — Play with Nova AI as your co-author
- **2-4 Players** — Narrator, Architect, Weaver, Wildcard roles
- **AI Story Sparks** — Nova breaks deadlocks with creative twists
- **Image Generation** — Dynamic character portraits and scene art
- **Multiplayer** — WebSocket-based room hosting/joining
- **Markdown Rendering** — Rich text in story responses
- **Character Tracking** — Floating stats with dynamic event detection

## Powered By

<a href="https://pollinations.ai">
  <img src="https://raw.githubusercontent.com/pollinations/pollinations/main/assets/logo-text.svg" alt="pollinations.ai" width="200" />
</a>

- **Text Generation** — [gen.pollinations.ai](https://pollinations.ai) (nova-fast model)
- **Image Generation** — [image.pollinations.ai](https://pollinations.ai) (Flux model)

## Build

```bash
./gradlew assembleDebug
```

## License

Apache 2.0
